...
export default function MemorialPagesPage({ dynamicId }) {
  ...
  // Load persistent memories for this specific memorial
  const [memories, setMemories] = useState(() => {
    try {
      const saved = localStorage.getItem(`hollowPineMemories_${dynamicId}`);
  ...
  useEffect(() => {
    if (dynamicId) {
      localStorage.setItem(`hollowPineMemories_${dynamicId}`, JSON.stringify(memories));
    }
  }, [memories, dynamicId]);
      ...
      <div className="site-container">
        ...
        {memories.length === 0 ? (
          ...
          <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
            {memories.map((memory) => (
              <div key={memory.id} className="break-inside-avoid bg-white rounded-2xl p-6 shadow-sm border border-[#E8DFD1] relative group hover:shadow-md transition-shadow">
                {/* Pin Icon */}
                <div className="absolute top-4 left-4 text-[#A8895C] bg-[#F8F6F0] p-1.5 rounded-full shadow-sm z-10">
                ...
                <div className={memory.image ? "mt-4 pt-2" : "mt-8"}>
                  {memory.text && (
                    <p className="text-[#3D3530] leading-relaxed mb-4 whitespace-pre-wrap font-serif italic text-lg">
                      "{memory.text}"
                    </p>
                  )}
        ...
        <Modal isOpen={showForm} onClose={() => setShowForm(false)} title="Pin a New Memory">
          <div className="space-y-5">
            ...
            <div>
              ...
              />
              {newMemory.image && (
                <div className="mt-2 text-xs text-green-600 font-semibold">✓ Image ready to pin</div>
              )}
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Share a favorite moment, a lesson learned, or a simple thought..."
                rows={5}
                className="w-full px-3 py-2 border border-[#E8DFD1] rounded-lg focus:outline-none focus:border-[#A8895C]"
              />
            </div>
            ...
            <div className="flex gap-3 pt-4">
              <Button variant="primary" onClick={handlePinMemory} className="flex-1 bg-[#1F2E27] hover:bg-[#A8895C] text-white">
                Pin to Board
              </Button>
