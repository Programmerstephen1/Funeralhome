...
export default function CartPage({ cart, updateQuantity, removeFromCart }) {
  ...
  // Helper function to reliably get the unique ID (Custom sized casket vs standard item)
  const getUniqueId = (item) => item.cartItemId || item.id;

  // When cart changes, default to selecting all items if they aren't already managed
  useEffect(() => {
    setSelectedItems(cart.map(item => getUniqueId(item)));
  }, [cart.length]);
  ...
  const deleteSelected = () => {
    selectedItems.forEach(uniqueId => removeFromCart(uniqueId));
    setSelectedItems([]);
  };
      ...
      <div className="site-container px-4 mx-auto max-w-5xl">
        ...
        <div className="flex flex-col gap-4 mb-8">
          {cart.map((item) => {
            ...
            return (
              <div key={uniqueId} className="bg-white border border-[#E8DFD1] p-4 md:p-6 flex flex-col md:flex-row items-start md:items-center gap-6 shadow-sm">
