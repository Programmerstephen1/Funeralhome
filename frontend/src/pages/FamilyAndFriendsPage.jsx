...
export default function FamilyAndFriendsPage({ dynamicId }) {
  ...
  const [newPerson, setNewPerson] = useState({ 
    ...
    role: "", 
    quote: "", 
    category: "family" // defaults to family
  });
  ...
  const handleAddPerson = () => {
    if (newPerson.name.trim() && newPerson.role.trim()) {
      ...
      if (newPerson.category === "family") {
        setFamily((prev) => [...prev, entry]);
      } else {
  ...
  return (
    <div className="bg-[#F8F6F0] py-12 min-h-screen">
      <div className="site-container max-w-6xl mx-auto">
        ...
        {/* PRO-GRADE FIX: The Return Button is now wired to the dynamicId */}
        <a 
        ...
        <div className="rounded-3xl bg-white border border-[#E8DFD1] p-10 mb-12 shadow-sm text-center">
          ...
          {/* UPDATED RETURN BUTTON TO MATCH THE SCREENSHOT FIX */}
          <a 
            href={`#memorial/${dynamicId || ''}`}
            className="inline-block border border-[#E8DFD1] text-[#3D3530] px-8 py-3 text-sm tracking-wider font-semibold rounded hover:border-[#A8895C] hover:text-[#A8895C] transition-colors"
          >
            Return to Memorial Hub
          </a>
        </div>

        {/* MODAL FOR ADDING FAMILY/FRIENDS */}
          ...
          <div className="space-y-5">
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input 
                    type="radio" 
                    name="category" 
                    checked={newPerson.category === "family"}
                    onChange={() => setNewPerson({ ...newPerson, category: "family" })}
                    className="text-[#A8895C] focus:ring-[#A8895C]"
                  />
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input 
                    type="radio" 
                    name="category" 
                    checked={newPerson.category === "friend"}
                    onChange={() => setNewPerson({ ...newPerson, category: "friend" })}
                    className="text-[#A8895C] focus:ring-[#A8895C]"
                  />
            ...
            <div>
              ...
              <input
                ...
                placeholder={newPerson.category === "family" ? "e.g., Granddaughter" : "e.g., Lifelong Friend"}
                className="w-full px-3 py-2 border border-[#E8DFD1] rounded-lg focus:outline-none focus:border-[#A8895C]"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="e.g., Her greatest joy and pride."
                rows={3}
                className="w-full px-3 py-2 border border-[#E8DFD1] rounded-lg focus:outline-none focus:border-[#A8895C]"
              />
            </div>
